...
Vous devez implémenter la fonction constructeur `Shape(type,color)`. Vous devez permettre à ce que l'attribut `type` soit modifiable après la création d'un objet seulement en modifiant la fonction `getType()` fournie.
...
Vous devez implémenter la fonction `RectangleFactory(obj, width, height)` qui retourne un nouvel objet de type `Object` qui contient tout ce que le paramètre `obj` contient déjà ainsi que les propriétés `width`, `height` et la fonction `getArea()` qui calcule la surface du rectangle.

## Partie 3 : Mixin

Vous devez implémenter la fonction `CircleMixin(obj, radius)` qui ajoute l'attribut `radius` et la fonction `getArea()` au paramètre `obj`.
