...
- Le formulaire devrait avoir une bordure noire d'une taille de `2px`.
...
- Le bouton `Connexion` devrait avoir un texte plus large que le reste du formulaire avec une couleur blanche sur un fond vert (valeurs RGB exactes laissées à votre choix).
...
Il est possible d'avoir plusieurs sélecteurs différents qui ciblent le même élément dans un document HTML. Vous devez donner 4 sélecteurs différents pour l'élément `<input>` de la ligne `35` du fichier [`index.html`](./index.html#l35).
...
Vous devez ajouter une règle CSS qui modifie le marqueur d'une entrée de la liste pour le symbole ★ d'une couleur différente lorsqu'on survole l'entrée avec votre souris et augmente sa taille. Lorsque la souris quitte l'entrée, le marqueur revient à sa taille, sa couleur et son symbole par défaut.
...
Pour voir l'effet visuel de l'exercice #3, vous devez survoler un des éléments de la liste de l'exercice #2 avec votre souris. Notez que vu la valeur exacte des propriétés à utiliser est laissée à votre choix, la réponse peut varier du code que vous avez écrit.